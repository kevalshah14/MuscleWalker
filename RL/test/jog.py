...
def main():
    ...
    model_path = "../../trained_models/ppo_muscle_jogger.zip"

    # Check if model exists
    if not os.path.exists(model_path):
        print(f"Model not found at {model_path}")
        print("Please run RL/train/jog.py first!")
        return

    print(f"Loading jogger model from {model_path}...")
    ...
    from train.jog import MuscleJoggerEnv

    # Create environment
    env = MuscleJoggerEnv(render_mode="human", terminate_on_fall=False)

    # Load the trained model
    model = PPO.load(model_path)

    print("\nStarting jogging simulation...")
    print("Press ESC in the viewer window to exit.")

    # Launch passive viewer with camera following the jogger
    with mujoco.viewer.launch_passive(env.model, env.data) as viewer:
        ...
        viewer.cam.distance = 4.0    # Slightly further back for faster jogging view
        ...
        while viewer.is_running():
            ...
            # Print jogging info occasionally
            if env.data.time % 0.5 < 0.05:  # Print every ~0.5 second
                ...
                # Show if achieving jogging speed target (1.2 m/s)
                speed_status = "✓ JOGGING" if forward_vel >= 1.0 else "~ FAST WALK" if forward_vel >= 0.7 else "✗ SLOW"
                print(f"  Status: {speed_status} | Phase: {jogging_phase:.2f}")

            # Sync viewer
            viewer.sync()

            # Slow down playback for better visualization (moderate speed for jogging)
            time.sleep(0.01)  # ~25 FPS for clear jogging visualization
